...
export default function ClassRoutine() {
  ...
  useEffect(() => {
    ...
    if (formData.class_id) {
      ...
      // Clear routine if class is deselected
      setRoutineData({})
    }
  }, [formData.class_id])
  ...
  const loadInitialData = async () => {
    try {
      const [progRes, roomRes, dayRes, periodRes, subjectRes, teacherRes] = await Promise.all([
        programmeService.getAll(),
        roomService.getAll(),
        dayService.getAll(),
        periodService.getAll(),
  ...
  const handleExport = () => {
    ...
    // Row 1: Kantipur Engineering College (centered, merged)
  ...
  const handleDeploy = async () => {
    try {
      ...
      console.log('Raw routines count:', rawRoutines.length)
      console.log('Grouped routines count:', groupedRoutines.length)
      ...
      // Send to backend to generate static page
      const response = await api.post('/deploy/static-page', deployData)
      
      if (response.data.success) {
        setSnackbar({
          open: true,
          message: `🚀 Deployed successfully! Students can access at: http://localhost:3001`,
          severity: 'success',
        })
      ...
      console.error('Error deploying routine:', error)
      setSnackbar({
        open: true,
        message: 'Failed to deploy. Please try again.',
        severity: 'error',
      })
    }
  }
                          ...
                          {periods.map((period) => {
                            ...
                            return (
                              <TableCell 
                                ...
                                onDragStart={(e) => hasContent && handleDragStart(e, day.id, period.id)}
                                onDragOver={(e) => handleDragOver(e, day.id, period.id)}
                                onDrop={(e) => handleDrop(e, day.id, period.id)}
                                onDragEnd={handleDragEnd}
                                sx={{ 
                                  cursor: hasContent ? 'move' : 'pointer',
                                ...
                                onClick={() => handleOpenAssignment(day.id, period.id, day.name, period.order)}
                              >
          ...
          <TabPanel value={tabValue} index={2}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', flexDirection: { xs: 'column', sm: 'row' } }}>
              ...
              <Button
                ...
                fullWidth
                sx={{
                  background: 'linear-gradient(45deg, #ff6b6b 30%, #ee5a6f 90%)',
                  color: 'white',
                  px: 3,
                  py: 1.5,
                  fontWeight: 600,
                  ...
                  maxWidth: { xs: '100%', sm: 'auto' },
                  '&:hover': {
                    background: 'linear-gradient(45deg, #ee5a6f 30%, #ff6b6b 90%)',
                    boxShadow: '0 4px 8px 3px rgba(255, 107, 107, .4)',
                  },
                }}
              >
                🚀 Deploy for Students
            ...
            <Paper elevation={0} sx={{ mt: 3, p: 2, bgcolor: '#f3f4f6', borderRadius: 2 }}>
              <Typography variant="body2" color="text.secondary" sx={{ fontSize: { xs: '0.75rem', sm: '0.875rem' } }}>
                ...
                📊 <strong>Format:</strong> Classes in rows, Time slots in columns<br />
                🚀 <strong>Deploy for Students:</strong> Creates a read-only web page accessible at http://localhost:3001 for students to view routines
              </Typography>
            </Paper>
          </TabPanel>
        </Paper>
